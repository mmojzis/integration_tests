...
for provider in list_providers('ec2'):
    ...
    delete_old_instances(ec2provider=ec2provider, date=datetime.now(), maxhours=args.maxhours,
                         excluded_instances=args.exclude_instances)
    sleep(120)
    logger.info("\nReleased addresses:")
    delete_disassociated_addresses(ec2provider=ec2provider, excluded_eips=args.exclude_eips)
    logger.info("\nDeleted volumes:")
    delete_unattached_volumes(ec2provider=ec2provider, excluded_volumes=args.exclude_volumes)
